...
require("./CompetitiveQuestionMasterFun")();


//Get question
router.get('/:inst/:questionid', async (req, res) => {
     
    let finalobj = await fetchcompquesdetails(req.params.inst, req.params.questionid);
    res.send(finalobj);
   // let resp = await insertcompdata(req.params.inst, finalobj);
    //res.send(resp);
});
...
router.post('/:inst', async (req, res) => {
    ...
    let resp = await insertcompdata(req.params.inst, req.body);
    res.send(resp)

});

//insert question function calling
const insertcompdata = async (inst, questionArray) => {
